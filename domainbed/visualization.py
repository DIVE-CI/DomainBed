...
class DeepFeatures(torch.nn.Module):
    ...
    def write_embeddings(self, x, outsize=(28, 28)):
        ...
        shutil.rmtree(self.embs_folder, ignore_errors=True)
        os.makedirs(self.imgs_folder, exist_ok=True)
        os.makedirs(self.embs_folder, exist_ok=True)
    ...
    def create_tensorboard_log(self, metadata=None, metadata_header=None):
        ...
        if metadata:
            self.writer.add_embedding(all_embeddings, metadata=metadata, metadata_header=metadata_header)
        else:
